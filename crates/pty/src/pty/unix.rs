    ...
    pub fn spawn(spec: &PtySpec) -> io::Result<Self> {
        ...
        unsafe {
            command.pre_exec(move || {
                ...
                // Resource caps, when the caller asked for them. Errors are
                // ignored deliberately: these are defence in depth, and a
                // machine that refuses the limit should still run the agent.
                if let Some((nofile, nproc)) = limits {
                    let file_limit = libc::rlimit {
                        rlim_cur: nofile,
                        rlim_max: libc::RLIM_INFINITY,
                    };
                    libc::setrlimit(libc::RLIMIT_NOFILE, &file_limit);
